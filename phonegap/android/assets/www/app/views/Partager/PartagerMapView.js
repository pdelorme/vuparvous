templates.partagerMapView = "app/views/Partager/PartagerMapView.html";
popupTemplate = 
	"<div class='popupContent'>" +
	"<div class='sitename'>" +
	"<%= site.get('site_name') %></div><div class='valider'>" +
	"<a href='#partager/<%= site.get('id') %>'>VALIDER</a>" +
	"</div></div>";
window.PartagerMapView = BaseView.extend({

    title: "Marseille VuParVous - Partager - Carte",
...
//    	"click #listButton":"gotoDecouvrirList",
...
//        "click #backButton":"back"
    ...
    render:function (eventName) {
        var template = _.template(templates.partagerMapView);
        var model = {
    ...
    initMap:function() {
        ...
        setTimeout( function() {
        	...
        	// appel̩ lors du clic sur site.s
            window.showDetail = function(siteId) {
                var view = new PartagerMainView({el:$("#contentView"),siteId:""+siteId});
            };
            ...
            // d̩fini une carte centr̩e sur ma position.
            var map = L.map('map', {
            ...
            var puTemplate = _.template(popupTemplate);
